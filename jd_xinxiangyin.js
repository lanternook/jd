...
!(async () => {
    let res = ['22cf267a2d774bd29ea2824d5e396b8c'];
    try{res = await getAuthorShareCode('http://192.168.9.151/xinxiangyin.json');}catch (e) {}
    if(!res){
        try{res = await getAuthorShareCode('http://192.168.9.151/xinxiangyin.json');}catch (e) {}
        if(!res){res = ['22cf267a2d774bd29ea2824d5e396b8c'];}
    }
    if (!cookiesArr[0]) {
...
async function main() {
    ...
    if(!$.activityData.opencard){
        if(!$.shareUuid){$.shareUuid = '22cf267a2d774bd29ea2824d5e396b8c';}
        await join($.venderId);
        await $.wait(2000);
        await takePostRequest('activityContent');
        await $.wait(2000);
